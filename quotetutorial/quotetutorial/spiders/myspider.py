...
class MySpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        data_css2 = response.meta.get('rule_contents')
        ...
        #     item['rule_contents'] = response.css(data_css2).getall()[i]
        #     yield item
            # Download the PDF file
           # pdf_link = response.css(data_css2).getall()[i]
           # self.download_pdf(pdf_link)
        for i, datum in enumerate(response.css(data_css2).css('::text').getall()):
            item = {}
            item['rule_contents'] = datum
            yield item
